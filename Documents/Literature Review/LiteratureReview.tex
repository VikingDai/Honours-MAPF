...
\noindent \textbf{Multi-agent Pathfinding.} In multi-agent pathfinding (MAPF) we look at a set of agents, $K$ where $location(k) \in V$. MAPF aims to find a path $p$ from start node $s$ to goal node $g$ for each agent in $K$. A path is a sequence specifying agent is on node $n$ at timestep $t$. Finally no path can conflict with another path.

Formally defining this:
	
\begin{itemize}
	\item $P$ is the set of all paths
	\item $\forall K (location(k) \in V)$
	\item $\forall K (\exists s (s \in V) \land \exists g(g \in V))$
	\item $\forall (k_{1} \in K) \forall (k_{2} \in K) (k_1 \neq k_2) (\exists p \land \forall K)$
	\item $\forall p_1 \in P, \forall p_2 \in P (\forall tl_1 \in p_1, \forall tl_2 \in p_2) tl_1 \neq tl_2$
\end{itemize}
...
\begin{enumerate}
	...
	\item Precomputating paths
\end{enumerate}

In this project we will be looking at \textit{Jump Point Search} within the area of pruning and
\textit{Compressed Path Databases} for pre-computation.
