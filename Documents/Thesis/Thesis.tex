...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Do
			...
			\State{$\textit{collisions} \gets failedAgents$}
...
\begin{algorithm}
	\caption{Temporal A* with collision penalties} \label{alg:temporalastarheur}
	\begin{algorithmic}[1]
		\Require $s:$ start, $g:$ goal, $\vec{c}:$ collisionPenalties, a vector of tile at time to penalty
		\Ensure $f:$ comparator
		\State{$f(n) = g(n) + h(n) + c(n)$}
		\Return $path$
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\If{$s$ or $g$ is not valid or $s$ equals $g$}
			\Return $path$
		\EndIf
		\State{$open \gets PriorityQueue()$}
		\State{add $TileTime(start)$ to $open$}
